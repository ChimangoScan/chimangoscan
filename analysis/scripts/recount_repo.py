...
if _FD_PATH:
    with open(_FD_PATH) as _fh:
        FILTER_DIGESTS = set(l.strip() for l in _fh if l.strip())
    sys.stderr.write("  [filter] corpus restricted to %d image_digests from %s\n"
...
if _FRT_PATH:
    with open(_FRT_PATH) as _fh:
        FILTER_RT = set(l.strip() for l in _fh if l.strip())
    sys.stderr.write("  [filter] corpus restricted to %d repo:tag from %s\n"
                     % (len(FILTER_RT), _FRT_PATH))
...
def main():
    ...
    # The exposure/pull/dependency_weight embedded in each report's meta were
    # written at ENQUEUE time by an older, buggy ranker that credited a base
    # image's downstream pulls to EVERY republication (e.g. i386/alpine got
    # alpine's 35e9 exposure). The corrected v2 ranking credits the downstream
    ...
    if os.path.exists(RANKING_V2):
        for _l in open(RANKING_V2):
            try:
                ...
                continue
            _ns = _o.get("repository_namespace", ""); _nm = _o.get("repository_name", "")
            _rt = (_nm if _ns == "library" else _ns + "/" + _nm) + ":" + _o.get("tag_name", "")
            if _rt not in v2_by_rt:
                v2_by_rt[_rt] = (_o.get("pull_count", 0) or 0, _o.get("exposure", 0) or 0,
                                 _o.get("dependency_weight", 0) or 0,
                                 _o.get("downstream_pull_sum", 0) or 0)
        sys.stderr.write("  [v2] corrected exposure loaded for %d repo:tag\n" % len(v2_by_rt))
    ...
    for image, rj, _nf, _fa in cur:
        ...
        # Corpus filter: keep only images whose digest is in the top-N head of
        # the corrected exposure ranking (drops low-exposure images scanned by
        # the earlier exposure bug). A report without a digest cannot be placed
        # in the ranking, so it is excluded when filtering is active.
        if FILTER_DIGESTS is not None and (not digest or digest not in FILTER_DIGESTS):
            continue
        if FILTER_RT is not None:
            _rt = image.split("@", 1)[0].strip()
            if _rt.startswith("library/"):
                _rt = _rt[8:]
            if _rt not in FILTER_RT:
                continue
        ...
        # Pull/exposure/dep-weight from the CORRECTED v2 ranking (keyed by
        # repo:tag), not from the buggy enqueue-time meta. Fall back to meta
        # only if a repo:tag is somehow absent from the ranking.
        _rt2 = image.split("@", 1)[0].strip()
        if _rt2.startswith("library/"):
            _rt2 = _rt2[8:]
        _v2 = v2_by_rt.get(_rt2)
        if _v2:
            pull, exposure, depw, dps = _v2
        else:
