#!/usr/bin/env python3
"""
Extra analyses for ChimangoScan paper:
  TASK A - oficiais vs comunidade
  TASK B - vulnerabilidades vs idade da tag

Single streaming read-only pass over reports table. Mongo lookup via ssh gpu1.
"""
import sqlite3, json, sys, os, math, time, datetime, subprocess

DB = "/media/kapelinski/BA1883111882CBB7/scanners_archive/ditector-D2/ditector.db"
...
def mongo_lookup(digests):
    """Look up last_updated for a set of digests in tags_data via ssh gpu1.
    ...
    for i in range(0, len(digests), BATCH):
        ...
        proc = subprocess.run(
            ["ssh", "gpu1", "docker", "exec", "-i", "ditector_mongo",
...
def make_fig_a(grp):
    ...
    ax = axes[0]
    bp = ax.boxplot([off, com], labels=["Oficiais\n(library)", "Comunidade"],
                    showfliers=False, patch_artist=True,
                    medianprops=dict(color="#222222", lw=1.5))
    for patch, c in zip(bp["boxes"], ["#9ecae1", "#fdae6b"]):
        patch.set_facecolor(c)
        patch.set_edgecolor("#444444")
    ax.set_ylabel("Vulnerabilidades (pkg-vuln) por imagem")
    ax.set_title("Distribuicao de vulnerabilidades")
    # annotate medians
    ...
    ax.set_title("Vazamento de secrets")
    ax.margins(y=0.20)
...
def make_fig_b(year_buckets):
    ...
    bars = ax.bar(x, medians, color="#3182bd", edgecolor="#444444",
                  label="Mediana de vulns/imagem")
    ax.plot(x, means, "o-", color="#d7301f", lw=1.6, ms=5,
            label="Media de vulns/imagem")
    ax.set_xticks(x)
    ax.set_xticklabels([str(y) for y in years])
    ax.set_xlabel("Ano de last_updated da tag (Docker Hub)")
    ax.set_ylabel("Vulnerabilidades (pkg-vuln) por imagem")
    ax.set_title("Vulnerabilidades vs. idade da tag")
...
def write_md(n_reports, grp, per_image, digest_to_lu, year_buckets,
    ...
    L = []
    w = L.append
    w("# ChimangoScan - Resultados extras (Tarefas A e B)\n")
    w(f"Data de geracao: 2026-05-17. Gerado por `extra_analyze.py`.\n")
    w(f"Fonte SQLite (read-only, `immutable=1`): `{DB}`")
    w(f"Fonte MongoDB (read-only): gpu1, container `ditector_mongo`, "
      f"db `dockerhub_data`, colecao `tags_data`.\n")
    w(f"Total de reports varridos na tabela `reports`: **{n_reports}**.\n")

    # ===== TASK A =====
    w("## Tarefa A - Imagens oficiais vs comunidade\n")
    w("Classificacao por `target.meta.repository_namespace`: OFICIAL se o "
      "namespace e `library` ou vazio/ausente; senao COMUNIDADE. "
      "Fonte: pass streaming unico sobre `reports.report_json`.\n")

    w("### A.1 Tamanho dos grupos")
    w("| Grupo | Imagens (reports) | % do total |")
    w("|---|---|---|")
    for label, key in [("Oficiais (library)", "official"),
                        ("Comunidade", "community")]:
        g = grp[key]
        w(f"| {label} | {g['n']} | {100.0*g['n']/n_reports:.2f}% |")
    w(f"| **Total** | **{n_reports}** | 100% |\n")

    w("### A.2 Distribuicao de findings `pkg-vuln` por imagem")
    w("Fonte: contagem de `findings[]` com `category=='pkg-vuln'` por report "
      "(findings merged, sem deduplicar entre scanners).\n")
    w("| Grupo | n imagens | min | mediana | media | p90 | p99 | max |")
    w("|---|---|---|---|---|---|---|---|")
    for label, key in [("Oficiais", "official"), ("Comunidade", "community")]:
    ...
    w("### A.3 Fracao de imagens com >=1 secret (`category=='secret'`)")
    w("Fonte: report tem >=1 finding `category=='secret'`.\n")
    w("| Grupo | imagens com secret | n imagens | fracao |")
    w("|---|---|---|---|")
    for label, key in [("Oficiais", "official"), ("Comunidade", "community")]:
        g = grp[key]
    ...
    # ===== TASK B =====
    w("## Tarefa B - Vulnerabilidades vs idade da tag\n")
    uniq = {d for d, _ in per_image if d}
    n_with_digest = sum(1 for d, _ in per_image if d)
    w(f"- Reports com `target.meta.image_digest` presente: **{n_with_digest}** "
      f"de {n_reports} (digests unicos: {len(uniq)}).")
    w(f"- Digests resolvidos em `tags_data` do Mongo: **{len(digest_to_lu)}**.")
    w(f"- Reports casados por digest com `last_updated` valido: "
      f"**{matched}** de {n_reports} "
      f"({100.0*matched/n_reports:.2f}%).\n")
    w("Fonte digest->last_updated: query `$in` em `tags_data` (campo "
      "`digest` casa com `target.meta.image_digest` dos reports). "
      "Idade = 2026-05-17 menos `last_updated`.\n")

    w("### B.1 Vulnerabilidades por ano de `last_updated`")
    w("Fonte: reports casados, agrupados pelo ano de `tags_data.last_updated`; "
      "vulns = findings `pkg-vuln` por imagem.\n")
    w("| Ano last_updated | n imagens | mediana vulns | media vulns | "
      "p90 vulns |")
    w("|---|---|---|---|---|")
    ...
    bands = [(0, 90), (90, 180), (180, 365), (365, 730), (730, 1460),
             (1460, 1e9)]
    band_labels = ["0-3 meses", "3-6 meses", "6-12 meses", "1-2 anos",
                   "2-4 anos", ">4 anos"]
    ...
    for age, nv in age_days:
        for (lo, hi), bl in zip(bands, band_labels):
            if lo <= age < hi:
                ...
                break
    w("### B.2 Vulnerabilidades por faixa de idade da tag")
    w("Fonte: mesmos reports casados, agrupados por idade "
      "(2026-05-17 - last_updated).\n")
    w("| Faixa de idade | n imagens | mediana vulns | media vulns | "
      "p90 vulns |")
    w("|---|---|---|---|---|")
    ...
    if age_days:
        ...
        w("### B.3 Tendencia")
        w(f"- n pares (idade_dias, vulns) = {n}")
        w(f"- correlacao de Pearson(idade_dias, vulns) = **{pearson:+.4f}**")
        w(f"- correlacao de Spearman(idade_dias, vulns) = **{spearman:+.4f}**")
        med_young = pct(sorted([v for a, v in age_days if a < 365]), 50)
        med_old = pct(sorted([v for a, v in age_days if a >= 365]), 50)
        w(f"- mediana vulns tags <1 ano: {med_young:.1f}; "
          f">=1 ano: {med_old:.1f}")
        trend = ("sobem" if spearman > 0.05 else
                 "descem" if spearman < -0.05 else
                 "sem tendencia clara")
        w(f"- **Interpretacao**: vulnerabilidades {trend} com a idade da tag "
