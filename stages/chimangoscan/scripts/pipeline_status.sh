#!/usr/bin/env bash
# pipeline_status.sh — counts for the ChimangoScan pipeline stages.
#
# Reads:
#   - Mongo (<your-host>, container chimangoscan_mongo)   → Stage I (crawl) + Stage II (build)
#   - build_metrics.log / build_checkpoint     → builder progress
#   - SQLite scanners/work/chimangoscan.db (<your-host>)  → scan queue (Stage III)
#
...
#            CHIMANGOSCAN_QUEUE_DB=<path>    (default: ${SCANNER_HOME}/scanners/work/chimangoscan.db)
#            CHIMANGOSCAN_METRICS=<path>     (default: ${SCANNER_HOME}/chimangoscan_research/build_metrics.log)
#            CHIMANGOSCAN_CHECKPOINT=<path>  (default: ${SCANNER_HOME}/chimangoscan_research/build_checkpoint.jsonl)

set -u

HOST="${DIT_HOST:-localhost}"
QUEUE_DB="${CHIMANGOSCAN_QUEUE_DB:-/var/lib/chimangoscan/chimangoscan.db}"
METRICS="${CHIMANGOSCAN_METRICS:-/var/lib/chimangoscan/build_metrics.log}"
CHECKPOINT="${CHIMANGOSCAN_CHECKPOINT:-/var/lib/chimangoscan/build_checkpoint.jsonl}"
...
mongo_eval() {
  run "docker exec chimangoscan_mongo mongosh --quiet --eval $(printf '%q' "$1")"
}

hr() { printf -- '── %s ──\n' "$1"; }

printf '============================================================\n'
printf '  ChimangoScan — pipeline status (host: %s)\n' "$HOST"
printf '  %s\n' "$(date -u +%FT%TZ)"
printf '============================================================\n\n'
...
printf '\n'
hr 'STAGE III — SCAN (queue scanners/work/chimangoscan.db)'
PY_QUEUE='
import sqlite3, sys
db = sys.argv[1]
