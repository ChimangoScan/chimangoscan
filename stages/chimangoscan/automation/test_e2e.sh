...
CRAWL_TIMEOUT="20"
BUILD_TIMEOUT="60"
OUTPUT="/tmp/chimangoscan_e2e_output.json"
BINARY="/tmp/chimangoscan_e2e"
E2E_DB="dockerhub_e2e"                   # ISOLATED database — does not conflict with the crawler
E2E_CONFIG="/tmp/chimangoscan_e2e_config.yaml"

_step() { echo ""; echo "[${1}] ${2}"; }
_ok()   { echo "      ✓ ${1}"; }
_fail() { echo "      ✗ ${1}"; exit 1; }

echo "=== ChimangoScan E2E Test  seed='${SEED}'  threshold=${THRESHOLD} ==="
...
cat > "$E2E_CONFIG" <<YAML
max_thread: 0
log_file: '/tmp/chimangoscan_e2e'
repo_with_many_tags_file: '/tmp/chimangoscan_e2e_repos.txt'
tmp_dir: '/tmp'
proxy:
  http_proxy: ''
  https_proxy: ''
