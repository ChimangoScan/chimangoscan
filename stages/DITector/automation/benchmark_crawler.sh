...
# The binary lives in /tmp/ditector (inside the container) — if the container is removed, it recompiles.
# To avoid recompiling: use an extra volume or leave the container stopped (not removed).
# Here: we compile to /app/ditector_bin, which is mounted on the .:/app volume
